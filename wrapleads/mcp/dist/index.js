...
server.registerTool('leads_list', {
    description: 'List leads in the WrapOS CRM. Filter by status, category, state, or search text. ' +
...
server.registerTool('leads_get', {
    ...
    const lead = await client.get(`/leads/${args.id}`);
    const result = { lead };
    if (args.include_activities) {
        try {
            const acts = await client.get(`/leads/${args.id}/activities`);
...
server.registerTool('leads_create', {
    description: 'Add a new lead to the WrapOS CRM. Use this after discovering a promising carrier via carriers_search, ' +
    ...
    const lead = await client.post('/leads/sync', [{
            clientId,
...
server.registerTool('leads_update', {
    description: 'Update a lead\'s status, notes, contact info, follow-up date, or tags. ' +
    ...
    if (rest.notes !== undefined)
        body.notes = rest.notes;
    if (rest.contact_name !== undefined)
        body.contactName = rest.contact_name;
    if (rest.contact_title !== undefined)
        body.contactTitle = rest.contact_title;
    if (rest.email !== undefined)
        body.email = rest.email;
    if (rest.phone !== undefined)
        body.phone = rest.phone;
    if (rest.website !== undefined)
        body.website = rest.website;
    if (rest.fleet_size !== undefined)
        body.fleetSize = rest.fleet_size;
    if (rest.followup_due_at !== undefined)
        body.followup_due_at = rest.followup_due_at;
    if (rest.tags !== undefined)
        body.tags = rest.tags;
    const result = await client.put(`/leads/${id}`, body);
    return { content: [{ type: 'text', text: ok('Lead updated', result) }] };
});
server.registerTool('leads_log_activity', {
    description: 'Log a manual activity on a lead — call notes, meeting outcomes, manual email record, ' +
    ...
    const result = await client.post(`/leads/${args.lead_id}/activities`, {
        type: args.type,
...
server.registerTool('leads_get_activities', {
    ...
    const data = await client.get(`/leads/${args.lead_id}/activities`);
...
server.registerTool('carriers_search', {
    description: 'Search the FMCSA database of 2M+ registered US fleet carriers. ' +
    ...
    if (args.max_fleet)
        params.set('maxFleet', String(args.max_fleet));
    if (args.query)
        params.set('q', args.query);
    if (args.cargo)
        params.set('cargo', args.cargo);
    params.set('limit', String(args.limit));
    params.set('offset', String(args.offset));
    const data = await client.post('/carriers/search', Object.fromEntries(params));
    return { content: [{ type: 'text', text: listResult('Carriers', data.carriers, data.total) }] };
});
server.registerTool('carriers_import', {
    description: 'Import a carrier from the FMCSA database into your WrapOS CRM as a new lead. ' +
    ...
    const result = await client.post('/carriers/import', {
...
server.registerTool('email_generate', {
    description: 'Generate an AI-written outreach email for a lead. Returns subject and body. ' +
    ...
    const lead = await client.get(`/leads/${args.lead_id}`);
    const settings = await client.get('/settings');
    const result = await client.post('/ai/email', {
        lead: lead.lead ?? lead,
...
server.registerTool('email_send', {
    description: 'Send an email to a lead via Resend. Requires RESEND_API_KEY to be configured in WrapOS. ' +
    ...
    const result = await client.post(`/leads/${args.lead_id}/send-email`, {
        subject: args.subject,
        body: args.body,
...
server.registerTool('email_generate_sequence', {
    description: 'Generate a multi-email outreach sequence (3-5 emails) for a lead. ' +
    ...
    const lead = await client.get(`/leads/${args.lead_id}`);
    const settings = await client.get('/settings');
    const result = await client.post('/ai/sequence', {
        lead,
        tone: args.tone,
        settings,
    });
...
server.registerTool('email_activate_sequence', {
    description: 'Activate an automated email drip sequence for a lead. ' +
    ...
    const result = await client.post(`/leads/${args.lead_id}/activate-sequence`, {
        tone: args.tone,
    });
...
server.registerTool('proposal_create', {
    description: 'Generate an AI-written proposal for a lead. Creates a shareable web page at a unique URL. ' +
    ...
    const result = await client.post(`/leads/${args.lead_id}/proposal`, { extra_notes: args.extra_notes ?? '' });
...
server.registerTool('proposal_list', {
    ...
    const data = await client.get(`/proposals${qs}`);
    let proposals = data.proposals;
    if (args.lead_id) {
        proposals = proposals.filter(p => p.lead_id === args.lead_id);
    }
...
server.registerTool('proposal_update_status', {
    ...
    if (args.expires_at)
        body.expires_at = new Date(args.expires_at + 'T23:59:59Z').toISOString();
    const result = await client.patch(`/proposals/${args.proposal_id}`, body);
...
server.registerTool('analytics_pipeline', {
    description: 'Get pipeline analytics: deal count and estimated value by status, win rate, ' +
        'average days to close, and top categories by revenue. Essential for pipeline reviews.',
    inputSchema: z.object({}),
    annotations: { readOnlyHint: true },
}, async () => {
    const data = await client.get('/leads/analytics');
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('analytics_overview', {
    description: 'Get a full analytics overview including pipeline health, email performance, top leads, ' +
...
server.registerTool('analytics_proposals', {
    description: 'Get proposal analytics: close rate, average time from created to sent, ' +
        'average days to approve, view counts for approved vs declined proposals, ' +
        'and top 5 most-viewed proposals.',
    inputSchema: z.object({}),
    annotations: { readOnlyHint: true },
}, async () => {
    const data = await client.get('/analytics/proposals');
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('analytics_email_performance', {
    description: 'Get email template performance analytics: open rates, send counts, and opens per template. ' +
        'Shows which email templates your prospects actually read.',
    inputSchema: z.object({}),
    annotations: { readOnlyHint: true },
}, async () => {
    const data = await client.get('/analytics/email-templates');
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('analytics_market_opportunity', {
    description: 'Get market penetration stats: how many carriers exist in each state, how many are in your CRM, ' +
        'and what percentage you\'ve reached. Shows untapped opportunity by geography.',
    inputSchema: z.object({}),
    annotations: { readOnlyHint: true },
}, async () => {
    const data = await client.get('/analytics/market-opportunity');
    return { content: [{ type: 'text', text: json(data) }] };
});
// ---------------------------------------------------------------------------
...
server.registerTool('jobs_list', {
    description: 'List installed wrap jobs. Each job tracks the lead, wrap category, vehicle count, ' +
    ...
    const params = new URLSearchParams();
    if (args.lead_id)
        params.set('leadId', String(args.lead_id));
    params.set('limit', String(args.limit));
    params.set('offset', String(args.offset));
...
server.registerTool('jobs_create', {
    description: 'Log a completed wrap installation. Triggers post-install care email and schedules ' +
...
server.registerTool('bids_list', {
    description: 'List open bids / quote requests. Bids track due dates, estimated value, win probability, ' +
    ...
    params.set('limit', String(args.limit));
    params.set('offset', String(args.offset));
    const data = await client.get(`/bids?${params}`);
...
server.registerTool('bids_create', {
    ...
    const result = await client.post('/bids', {
        leadId: args.lead_id,
        title: args.title,
...
server.registerTool('mission_followups', {
    description: 'Get leads due for follow-up today or overdue. Returns leads sorted by urgency — ' +
        'most overdue first. Essential for daily outreach planning.',
    inputSchema: z.object({}),
    annotations: { readOnlyHint: true },
}, async () => {
    const data = await client.get('/leads/needs-followup');
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('mission_hot_opens', {
    description: 'Get leads who recently opened a tracked email. Sorted by recency and open count. ' +
        'Use this to prioritize who to call next — they\'re thinking about your pitch right now.',
    inputSchema: z.object({}),
    annotations: { readOnlyHint: true },
}, async () => {
    const data = await client.get('/mission/hot-opens');
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('mission_today_score', {
    description: 'Get today\'s activity score: emails sent, calls logged, leads advanced, and deals won. ' +
        'Shows progress toward daily outreach goals.',
    inputSchema: z.object({}),
    annotations: { readOnlyHint: true },
}, async () => {
    const data = await client.get('/mission/today-score');
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('mission_news_signals', {
    description: 'Get recently auto-created leads from the News Signal worker — fleet expansions, ' +
    ...
    const data = await client.get(`/mission/news-signals?limit=${args.limit}`);
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('analytics_time_to_close', {
    description: 'Get average days-to-close broken down by deal category (fleet, design, construction, etc.). ' +
    ...
    const data = await client.get('/analytics/time-to-close');
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('leads_find_email', {
    description: 'Run email permutation + optional SMTP probe on a single lead. ' +
    ...
    const data = await client.get(`/leads/${args.lead_id}/find-email?probe=${args.probe ?? false}`);
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('leads_get_contacts', {
    description: 'Get all contacts (decision-makers) associated with a lead. ' +
    ...
    const data = await client.get(`/leads/${args.lead_id}/contacts`);
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('leads_add_contact', {
    description: 'Add a new contact (decision-maker) to a lead. ' +
    ...
    const data = await client.post(`/leads/${lead_id}/contacts`, body);
    return { content: [{ type: 'text', text: ok('Contact added', data) }] };
});
server.registerTool('proposal_coach', {
    description: 'Get AI coaching on how to improve a proposal before sending. ' +
    ...
    if (args.proposal_id)
        params.set('proposal_id', String(args.proposal_id));
    const data = await client.get(`/ai/proposal-coach?${params}`);
    return { content: [{ type: 'text', text: json(data) }] };
});
server.registerTool('analytics_cohort', {
    description: 'Get cohort analysis — leads grouped by month added, showing win rate, 90-day close rate, ' +
    ...
    const data = await client.get('/analytics/cohort');
    return { content: [{ type: 'text', text: json(data) }] };
});
// ---------------------------------------------------------------------------
// Start
