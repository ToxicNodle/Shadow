...
try {
  ...
  // Real public-data leads — generated by scripts/pull-real-leads.js.
  // Wrapped in try/catch so a missing file (e.g. on a fresh clone before
  // the puller has been run) doesn't break autoSeed.
  let epa = [], usaSpend = [], osm = [];
  try { epa      = require(path.join(__dirname, '../seed-epa-facilities')).LEADS;    } catch { /* not generated yet */ }
  try { usaSpend = require(path.join(__dirname, '../seed-usaspending-leads')).LEADS; } catch { /* not generated yet */ }
  try { osm      = require(path.join(__dirname, '../seed-osm-industrial')).LEADS;    } catch { /* not generated yet */ }
  ALL_LEADS = [
    ...(leads||[]), ...(gc||[]), ...(designers||[]), ...(schools||[]),
    ...(racing||[]), ...(solar||[]),
    ...(epa||[]), ...(usaSpend||[]), ...(osm||[]),
  ];
